...
local binserialize = require("binserialize")
local api = {}
...
function api.serializeRecursiveTable(T, compact, bin)
  expect(1, T, "table")
  if type(compact) ~= "boolean" then compact = true end
  local tmpT = flattenRecursiveTableClone(T)
  if bin then
    return binserialize.serialise(tmpT)
  end
...
function api.saveRecursiveTable(T, filename, compact)
  ...
  if f then
    f.write(api.serializeRecursiveTable(T,compact,true))
    f.close()
    return true
  end
  return false
...
function api.loadRecursiveTable(filename, metaT)
  ...
  if f then
    local T = binserialize.unserialise(f.readAll())
    recurseFlatTable(T)
    if (metaT) then
